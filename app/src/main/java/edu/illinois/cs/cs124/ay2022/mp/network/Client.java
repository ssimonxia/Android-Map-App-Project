...
public final class Client {
    ...
    StringRequest postFavoritePlaceRequest =
        new StringRequest(
            ...
            response -> {
              ...
              /*
               * The TypeReference<>() {} is a bit of magic required to have Jackson
               * return boolean to make sure whether post is success
               * We wrap this in a try-catch to handle deserialization errors that may occur.
               */
              // Pass the Boolean.TRUE to callback, since post is always success at this point
          ...
          @Override
          public byte[] getBody() {
            // Serialize the object that is sent to the server
            String p = "";
            try {
              p = OBJECT_MAPPER.writeValueAsString(place);
            } catch (JsonProcessingException error) {
              assert true;
            }
  ...
  public void postComment(
    ...
    StringRequest postFavoritePlaceRequest =
        new StringRequest(
            Request.Method.POST,
            FavoritePlacesApplication.SERVER_URL + "/comment/",
            response -> {
              // This code runs on success
              /*
               * The TypeReference<>() {} is a bit of magic required to have Jackson
  ...
  public void postPlan(
    ...
    StringRequest postFavoritePlaceRequest =
        new StringRequest(
            Request.Method.POST,
            FavoritePlacesApplication.SERVER_URL + "/plan/",
            response -> {
              // This code runs on success
              /*
               * The TypeReference<>() {} is a bit of magic required to have Jackson
